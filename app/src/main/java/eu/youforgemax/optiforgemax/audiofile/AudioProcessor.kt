...
/**
 * Offline audio-file normalizer. Works for any container that carries an audio
 * track, including VIDEO files — the audio track is extracted, processed, and
 * written back out as an AAC .m4a (the video stream is discarded).
 *
 *   pass 1 (optional): decode audio -> measure integrated loudness -> target gain
 *   pass 2: decode audio -> 5-band comp/limiter (+ target gain) -> AAC encode -> mux .m4a
 *
 * Encoded audio is buffered in memory before muxing; fine for typical clips,
...
object AudioProcessor {
    ...
    fun process(
        ...
        // --- mux: write encoded audio to an .m4a -----------------------------
        progress("Writing output", 0.92f)
        writeAudio(ctx, outputUri, aacFormat, encoded)
        progress("Done", 1f)
    }
    ...
    /**
     * Decode the audio track (of an audio OR video container) to deinterleaved
     * float blocks. onBlock(block[ch][frames], frames, sampleRate). onFormat(sr, ch) first.
     */
