...
/**
 * Single observable source of truth for the audio-file mode controls. Compose
 * reads/writes it; snapshot() freezes it for the worker; JSON (de)serialises presets.
 */
class UiState {
...
/** Named presets in SharedPreferences (one JSON blob per name). Separate store from the live app. */
class PresetRepo(ctx: Context) {
    private val sp = ctx.getSharedPreferences("presets_audiofile", Context.MODE_PRIVATE)
