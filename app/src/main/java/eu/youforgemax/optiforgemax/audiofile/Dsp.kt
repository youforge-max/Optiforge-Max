...
/**
 * Offline software port of the live app's 5-band compressor / limiter, for
 * processing audio FILES (open > preview > save).
 *
 * The live app uses android.media.audiofx.DynamicsProcessing — a realtime effect
 * on the output mix with no file-render API. To process a file we reimplement the
 * same chain in pure Kotlin on PCM:
 *
 *   input gain -> LR4 5-band crossover -> per-band soft-knee compressor
 *              -> makeup -> sum -> limiter (+ brickwall ceiling)
 *
 * Ported from youforge-max's video normalizer so presets/behaviour stay consistent.
 */
